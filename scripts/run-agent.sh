...
if [ "$PROVIDER" = "codemie" ]; then
  ...
  if [ ${#PASS_ARGS[@]} -eq 0 ]; then
    CMD=(claude --permission-mode auto-accept --model "$MODEL" --max-turns 50 --dangerously-skip-permissions -p "$PROMPT_FILE")
  else
    CMD=(claude --permission-mode auto-accept --model "$MODEL" "${PASS_ARGS[@]}" --max-turns 50 --dangerously-skip-permissions -p "$PROMPT_FILE")
  fi
